...
# Define output path based on script location (writable location)
script_dir = Path(__file__).parent
output_dir = script_dir / "gaz_index_html"
output_dir.mkdir(parents=True, exist_ok=True)
output_file = output_dir / "index.html"

df = pd.read_excel('/Users/fabiorakoto/Desktop/GAZ MAP DATABASE/_GAZ DATA BASE QUEBEC.xlsx')
...
# Add LocateControl for functionality
plugins.LocateControl(auto_start=False, position='topleft').add_to(gazmap)
...
for idx, row in df.iterrows():
    ...
    #contexte de guerre en Iran, prix en hausse, seuils ajustés pour refléter la réalité actuelle du marché
    try:
        price_value = float(price.replace("$", "").strip()) if price else 0
        # Green for <= 130$, Orange for 130-135$, Red for > 135$
        if price_value >=175:
            marker_color = "#EE0E0E"  # Red
        elif price_value >=165:
    ...
    legend_html = """
        <div style="
            ...
            min-width: 180px;
        ">
            <b style="font-size: 14px;">Prix de l'essence 💵 mis a jour le 06/06/2026</b><br><br>
            <span style="display:inline-block; width:14px; height:14px; background:#50D42F; border-radius:3px; margin-right:8px; vertical-align:middle;"></span>
            Moins de 165 ¢/L<br><br>
            <span style="display:inline-block; width:14px; height:14px; background:#FF8C00; border-radius:3px; margin-right:8px; vertical-align:middle;"></span>
            Entre 165 ¢/L et 175 ¢/L<br><br>
            <span style="display:inline-block; width:14px; height:14px; background:#EE0E0E; border-radius:3px; margin-right:8px; vertical-align:middle;"></span>
            175 ¢/L et plus
        </div>
        """
    gazmap.get_root().html.add_child(folium.Element(legend_html))
